...
% Converts all the time-series in a Simulink simulation 
% output to a table of data with the same column names.
%
% The simulation outputs may be vectors, arrays, timeseries
% objects, or structs. For structs and timeseries, the time
% data is compared to the time vector t provided. For arrays,
% any array which has the same length as t will be included.
%
% Data arrays (i.e. 2 dimensional) are given labels with
% a numerical subscript. For example, if 'x' is an
% array of dimensions nT * 3, then the columns of the
